...
const CategoryItem = ({ id, title, subtitle, foods, onFoodItemClick }) => {
  const betweenXSandSM = ReponsiveQueryBetween("xs", "sm");
  const betweenSMandMD = ReponsiveQueryBetween("sm", "md");

  const xsItem = () => {
    if (betweenXSandSM) return 12;
    if (betweenSMandMD) return 6;
    return 4;
  };
    ...
    <div className="category-item">
      ...
      <Grid container spacing={2} mt={2}>
        {_.map(foods, (food, idx) => {
          return (
            <Grid xs={xsItem()}>
              <FoodItem {...food} onFoodItemClick={onFoodItemClick} />
            </Grid>
          );
        })}
      </Grid>
